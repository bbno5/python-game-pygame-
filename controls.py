...
def create_army(screen, inos):
    ...
    for i in range(number_ino_y):
        for j in range(number_ino_x):
            ino = Ino(screen)
            if count == 0:
                ino.x = i*ino_width + (ino_width * (i+1))
                ...
                ino.x = i*ino_width + (ino_width * (i+2))
                ino.y = ino_height + (ino_height * j)
                ino.rect.x = ino.x
                ino.rect.y = ino.y
                count-=1
    ...
    # while i < number_ino_y:
    #     while j < number_ino_x:
    #         if count == 1:
    #             ino = Ino(screen)
